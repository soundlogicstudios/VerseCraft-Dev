...
/* ============================================================
   ...
   - Updates #vc-bg background-image based on current story position
   - Priority: Section/Node override → Node default → Act default
   - Space-canon supported: "Act 01", "Node 0101" => URL-encoded automatically
   - Tries extensions in order: webp → png → jpg → jpeg
   ============================================================ */

/**
 * Base for raw GitHub media (change only these two constants if your repo/path moves).
 * Example:
 *   MEDIA_GITHUB_RAW = "https://raw.githubusercontent.com/<owner>/VerseCraft-Media/<branch>/"
 *   MEDIA_ROOT       = "Media/"   (or whatever folder contains Stories/, SFX/, etc.)
 */
const MEDIA_GITHUB_RAW =
  "https://raw.githubusercontent.com/soundlogicstudios/VerseCraft-Media/main/";
const MEDIA_ROOT = "Media/"; // <-- you’ll replace this if your folder path differs
...
function mediaUrl(relPath){
  // relPath should NOT start with "/" (we normalize just in case)
  const rp = String(relPath || "").replace(/^\/+/, "");
  return encPath(MEDIA_GITHUB_RAW + MEDIA_ROOT + rp);
}
...
function buildNodeBgUrls({ storyId, actId, nodeId }){
  ...
  return exts.map(ext =>
    mediaUrl(`Stories/${storyId}/Acts/${actId}/Nodes/${nodeFolder}/Scenes/Background.${ext}`)
  );
}
...
function buildActDefaultBgUrls({ storyId, actId }){
  const exts = ["webp","png","jpg","jpeg"];
  return exts.map(ext =>
    mediaUrl(`Stories/${storyId}/Acts/${actId}/Defaults/Scenes/Background.${ext}`)
  );
}

/**
 * Optional per-section overrides:
 * In your story JSON section, you can add any of these:
 *   - bg: "myfile.png"                (resolved inside the current Node Scenes/)
 *   - background: "Scenes/Foo.webp"   (resolved under current Node folder unless it starts with Stories/)
 *   - bg: "https://..."               (full URL supported)
 *
 * If the value has no extension, we try webp/png/jpg/jpeg.
 */
function sectionBgSpec(section){
  if(!section) return null;
  const v =
    section.bg ??
    section.background ??
    section.sceneBg ??
    section.sceneBackground ??
    section?.scene?.bg ??
    section?.scene?.background ??
    null;
...
function buildSectionOverrideUrls({ storyId, actId, nodeId, section }){
  const spec = sectionBgSpec(section);
  if(!spec) return [];
  ...
  const exts = ["webp","png","jpg","jpeg"];
  const hasExt = /\.(webp|png|jpg|jpeg)$/i.test(spec);

  // Normalize relative paths
  const cleaned = spec.replace(/^\/+/, "");

  // If they specify a full repo-relative path (under MEDIA_ROOT)
  // Example: "Stories/<id>/Acts/Act 01/Defaults/Scenes/Background.png"
  if(cleaned.startsWith("Stories/")){
    if(hasExt) return [mediaUrl(cleaned)];
    return exts.map(ext => mediaUrl(`${cleaned}.${ext}`));
  }

  // If we can’t infer node, fall back to Act default resolution only
  if(nodeId == null){
    if(hasExt) return [mediaUrl(`Stories/${storyId}/Acts/${actId}/Defaults/${cleaned}`)];
    return exts.map(ext => mediaUrl(`Stories/${storyId}/Acts/${actId}/Defaults/${cleaned}.${ext}`));
  }

  const nodeFolder = nodeFolderFromId(nodeId);

  // If they gave just a filename, assume it lives in Node Scenes/
  if(!cleaned.includes("/")){
    if(hasExt){
      return [mediaUrl(`Stories/${storyId}/Acts/${actId}/Nodes/${nodeFolder}/Scenes/${cleaned}`)];
    }
    return exts.map(ext =>
      mediaUrl(`Stories/${storyId}/Acts/${actId}/Nodes/${nodeFolder}/Scenes/${cleaned}.${ext}`)
    );
  }

  // Otherwise treat it as a path relative to the Node folder (common for Scenes/Whatever)
  if(hasExt){
    return [mediaUrl(`Stories/${storyId}/Acts/${actId}/Nodes/${nodeFolder}/${cleaned}`)];
  }
  return exts.map(ext =>
    mediaUrl(`Stories/${storyId}/Acts/${actId}/Nodes/${nodeFolder}/${cleaned}.${ext}`)
  );
}
...
async function updateBackgroundForState(){
  try{
    ...
    // include sectionId so overrides that change per section won’t get “stuck”
    const key = `${storyId}::${actId}::${nodeId ?? "NO_NODE"}::${state.sectionId ?? ""}`;
    if(key === LAST_BG_KEY) return;

    let urls = [];

    // 1) Optional per-section / per-node override (most specific)
    urls = urls.concat(buildSectionOverrideUrls({ storyId, actId, nodeId, section }));

    // 2) Node default (Background.* under Node Scenes/)
    if(nodeId !== null){
      urls = urls.concat(buildNodeBgUrls({ storyId, actId, nodeId }));
    }

    // 3) Act default (Background.* under Act Defaults/)
      ...
      <div class="vc-panel vc-scroll">
        ...
        <div class="vc-content">
          <div class="vc-scene">
            ...
            <div class="vc-sceneSub">Now auto-loads from VerseCraft-Media (Section/Node → Node → Act fallback)</div>
          </div>
