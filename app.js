...
function buildNodeBgUrls({ storyId, nodeId }){
  const nodeFolder = nodeFolderFromId(nodeId);
  const exts = ["webp","png","jpg","jpeg"];
  return exts.map(ext => encPath(
    MEDIA_BASE +
      `Stories/${storyId}/Nodes/${nodeFolder}/Scenes/Background.${ext}`
  ));
}


function buildStoryDefaultBgUrls({ storyId }){
  const exts = ["webp","png","jpg","jpeg"];
  return exts.map(ext => encPath(
    MEDIA_BASE +
      `Stories/${storyId}/Defaults/Scenes/Background.${ext}`
  ));
}
...
async function updateBackgroundForState(){
  try{
    ...
    let urls = [];
    if(nodeId !== null){
      urls = urls.concat(buildNodeBgUrls({ storyId, nodeId }));
    }
    urls = urls.concat(buildStoryDefaultBgUrls({ storyId }));
